...
module SandthornDriverSequel
  describe EventStore do
    ...
    #     e << {aggregate_version: 1, event_name: "new", event_args: nil, event_args: data}
    #   end
    #   let(:aggregate_id_1) {"c0456e26-e29a-4f67-92fa-130b3a31a39a"}
    ...
    #     expect(MessagePack.unpack(events.first[:event_args], symbolize_keys: true)).to eql test_data
    #   end
    ...
    context "when saving events that have no aggregate_version" do
      let(:test_events) do
        e = []
        e << {aggregate_version: nil, event_name: "new", event_args: nil, event_args: {:method_name=>"new", :method_args=>[], :attribute_deltas=>[{:attribute_name=>"aggregate_id", :old_value=>nil, :new_value=>"0a74e545-be84-4506-8b0a-73e947856327"}]}}
        e << {aggregate_version: nil, event_name: "foo", event_args: ["bar"], event_args: "noop"}
        e << {aggregate_version: nil, event_name: "flubber", event_args: ["bar"] , event_args: "noop"}
      end
